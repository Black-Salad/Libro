const App = (() => {
        ...
        if (e.target.closest(".dropdown-toggle")) {
          ...
          if (nav.dataset.accordion != "false") {
            ...
            nav
              ...
              .forEach(
                (shown) => el != shown && shown.classList.remove("show")
              );
          }
    ...
    void (function () {
      ...
      if (sidebar) {
        ...
        document.addEventListener("click", (e) => {
          if (e.target.closest('[data-toggle="sidebar"]')) {
            if (
              !bodyClass.contains("sidebar-collapse") ||
              bodyClass.contains("sidebar-expand")
            ) {
  ...
  function customFileInput() {
    document.addEventListener("change", (e) => {
      if (e.target.closest(".custom-file-input")) {
        ...
        const chooseText = el.dataset.choose
          ? el.dataset.choose
        ...
        switch (el.dataset.action) {
          ...
          case "reload":
            card.insertAdjacentHTML(
              "afterbegin",
              '<div class="card-loader-overlay"><div class="spinner-border text-white" role="status"></div></div>'
            );
            card.dispatchEvent(new Event("card:reload"));
            break;
          case "collapse":
            const collapsingTransition =
              parseFloat(
                getComputedStyle(document.querySelector(".collapsing"))[
                  "transitionDuration"
                ]
              ) * 1000;
            setTimeout(() => {
              if (
                document
                  .querySelector(el.dataset.target)
    ...
    document.addEventListener("click", (e) => {
      if (e.target.closest("#navSection")) {
        ...
        const y =
          ...
          window.pageYOffset -
          ((document.body.dataset.offset || 140) - 1);
  ...
  function checkAll() {
    if (document.querySelectorAll(".has-checkAll").length) {
      ...
      for (const el of document.querySelectorAll(".has-checkAll")) {
        ...
        const tdCheckbox = el.querySelectorAll(
        ...
        thCheckbox.addEventListener("click", function () {
          for (const cb of tdCheckbox) {
            ...
            cb.checked
              ? cb.closest("tr").classList.add(activeClass)
              : cb.closest("tr").classList.remove(activeClass);
          }
        ...
        for (const cb of tdCheckbox) {
          cb.addEventListener("click", function () {
            this.checked
              ? this.closest("tr").classList.add(activeClass)
              : this.closest("tr").classList.remove(activeClass);
            bulkTarget && toggleBulkDropdown(bulkTarget, tdCheckbox);
          });
        }
      }
      function toggleBulkDropdown(el, tdCheckbox) {
        ...
        bulk_dropdown.querySelector(".checked-counter") &&
          (bulk_dropdown.querySelector(".checked-counter").textContent = count);
        count != 0
          ? bulk_dropdown.removeAttribute("hidden")
          : bulk_dropdown.setAttribute("hidden", "");
      }
    }
  }
  ...
  function backgroundCover() {
    document
      ...
      .forEach((el) => (el.style.backgroundImage = `url(${el.dataset.cover})`));
  }
  ...
  function scrollNavbar() {
    if (document.querySelector(".main-navbar")) {
        ...
        resize(() => {
          if (lgUp()) {
            for (const el of document.querySelectorAll("[data-scroll]")) {
              if (
                navbar.querySelector(".navbar-nav").getBoundingClientRect()
      ...
      $(".main-navbar .dropdown").on("show.bs.dropdown", function () {
        let margin = document.querySelector(".main-navbar .navbar-collapse")
          .scrollLeft;
  ...
  function summernoteFocus() {
    $(document)
      .on("summernote.focus", ".summernote", function () {
  ...
  function toast(option) {
    const animation =
      option.animation !== undefined ? option.animation : "true";
    const autohide = option.autohide !== undefined ? option.autohide : "true";
    const position =
      option.position !== undefined ? option.position : "top-right";
    ...
    // Icon
    const iconSuccess =
      '<svg class="mr-2 text-success" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="21" height="21"><path d="M0 0h24v24H0z" fill="none"/><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>';
    const iconWarning =
      '<svg class="mr-2 text-warning" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="21" height="21"><path d="M0 0h24v24H0z" fill="none"/><path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/></svg>';
    const iconError =
      '<svg class="mr-2 text-danger" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="21" height="21"><path d="M0 0h24v24H0z" fill="none"/><path d="M0 0h24v24H0z" fill="none"/><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>';
    const iconInfo =
      '<svg class="mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="21" height="21"><path d="M0 0h24v24H0z" fill="none"></path><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z"></path></svg>';
    let icon = "";
    switch (option.icon) {
      case "success":
        icon = iconSuccess;
    ...
    if (!document.querySelector(wrapper)) {
      document.body.insertAdjacentHTML(
        "beforeend",
        `<div class="toast-wrapper ${position}"></div>`
      );
    }
  ...
  function autowidth() {
    if (document.querySelectorAll(".autowidth-hidden").length) {
      document
        .querySelectorAll(".autowidth-hidden")
    ...
    function setWidth(el, fakeEl) {
      ...
      el.style.setProperty(
        "width",
        Math.ceil(window.getComputedStyle(fakeEl).width.replace("px", "")) +
          1 +
          "px",
        "important"
      );
