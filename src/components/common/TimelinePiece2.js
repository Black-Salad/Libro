...
const TimelinePiece2 = ({ timeline, onOpenModal }) => {
  ...
  return (
    <Paper className={classes.paper}>
      ...
      <div className={`media`}>
        ...
        >
          <b>{convDateStr(timeline.shelf_id.start_date)}</b> ~{" "}
          <b>{convDateStr(timeline.shelf_id.end_date)}</b> 완독 👍
        </div>
      </div>
    </Paper>
  );
